...
This lesson introduces students to coastal hazard exposure in Puerto Rico using demographic and spatial data. You will use the [Low Elevation Coastal Zones (LECZ)](https://sedac.ciesin.columbia.edu/data/set/lecz-low-elevation-coastal-zones) dataset from NASA SEDAC and census block group data from the [US Census Bureau](https://www.census.gov/data.html) via the [IPUMS](https://www.ipums.org/) National Historical Geographic Information System (NHGIS) portal to understand which populations live in low-lying areas vulnerable to flooding and sea level rise. 
...
- Perform a **spatial overlay** with LECZ boundaries to assess exposure.
- Use `ArcGIS` and `folium` Python packages to explore spatial patterns interactively.
...
You may wonder why studies of the “entire” US often restrict themselves to the CONUS? The simplest answer: data or computational limitations. Some U.S. territories lack full census variable coverage. 
...
This lesson helps address these issues by showing how to **combine demographic data with geospatial boundaries and Low Elevation Coastal Zones (LECZs)** to assess who is most at risk from coastal hazards in **Puerto Rico**.
...
are **disproportionately** located within these low-lying zones.  

For example, **1 in 5 urban Black residents** lives in the 10m LECZ [@tagtachian2023; @hauer2020sea].
...
from ipumspy import AggregateDataExtract, ddi
...
### Downloading shapefiles from IPUMS
...
sns.pairplot(
    ...
    plot_kws=dict(marker="o", facecolors='none', edgecolor='blue'),
    height=1.7
)

# Add overall title and adjust layout
plt.suptitle("Pairwise Scatterplots and Histograms (2010 to 2020 Changes)", y=1.02)
plt.tight_layout()
plt.show()
...
# Anonymous GIS connection (with SSL disabled if needed)
gis = GIS( )

# Directly connect to the FeatureLayer by URL
url = "https://services2.arcgis.com/IsDCghZ73NgoYoz5/arcgis/rest/services/US_county_2020/FeatureServer/0"
layer = FeatureLayer(url)

# Query Puerto Rico counties (STATEFP = '72')
sedf = layer.query(where="STATEFP = '72'").sdf
```
```{python}
...
# pr_ipums_gdf.set_crs(epsg=4326) 
```
...
# Drop NaNs in geometry and target field
gdf = pr_ipums_gdf.dropna(subset=["geometry", "Pop60plus_dz"]).reset_index(drop=True)
gdf = gdf.to_crs(epsg=4326)
gdf["id"] = gdf.index.astype(str)

# Map center from bounding box
minx, miny, maxx, maxy = gdf.total_bounds
center = [(miny + maxy) / 2, (minx + maxx) / 2]

# Compute quantile breaks
field = "Pop60plus_dz"
breaks = list(np.quantile(gdf[field], np.linspace(0, 1, 6)))
...
folium.Choropleth(
    ...
    legend_name="Z-score of Change in Population Aged 60+, 2010 to 2020"
).add_to(m)
...
# Prepare base demographic layer
gdf = pr_ipums_gdf.dropna(subset=["geometry", "Pop60plus_dz"]).reset_index(drop=True)
gdf = gdf.to_crs(epsg=4326)
gdf["id"] = gdf.index.astype(str)

# Prepare overlay LECZ layer (must also be in EPSG:4326)
lecz_gdf = lecz_union_gdf.dropna(subset=["geometry"]).to_crs(epsg=4326)
...
# ---- Layer 1: Choropleth of 60+ Z-scores ----
...
def plot_pyramid_stacked_by_lecz(pyr2010, pyr2020): 
    ...
    fig, (ax1, ax2) = plt.subplots(nrows=2,  sharex=True, figsize=(6.5, 7.5))
